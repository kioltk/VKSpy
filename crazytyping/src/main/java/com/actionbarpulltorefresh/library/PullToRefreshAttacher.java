...
public class PullToRefreshAttacher {
    ...
    final void setRefreshFailed(){
        //setRefreshing(false);

        mHeaderTransformer.onReset();
        mHeaderTransformer.onRefreshFailed();
        ...
        if (mRefreshMinimize) {
            if (mRefreshMinimizeDelay > 0) {
                getHeaderView().postDelayed(mRefreshEndRunnable, mRefreshMinimizeDelay*2);
            } else {
                getHeaderView().post(mRefreshEndRunnable);
            }
        }
    }
    /**
    ...
    private final Runnable mRefreshEndRunnable = new Runnable() {
        @Override
        public void run() {
            //mRefreshMinimizeRunnable.run();
            setRefreshComplete();
        }
    };
